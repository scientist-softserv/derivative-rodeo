...
RSpec.describe SpaceStone::Derivatives::TextExtractors::Ocr do
  let(:altoxsd) do
  ...
  let(:example_gray_tiff) { Fixtures.path_for('ocr_gray.tiff') }
  let(:example_mono_tiff) { Fixtures.path_for('ocr_mono.tiff') }
  let(:example_color_tiff) { Fixtures.path_for('ocr_color.tiff') }
  let(:example_gray_jp2) { Fixtures.path_for('ocr_gray.jp2') }
