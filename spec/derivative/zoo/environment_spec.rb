...
RSpec.describe Derivative::Zoo::Environment do
  ...
  describe '.for_mime_type_processing' do
    ...
    it "builds a chain for the given mime_type" do
      expect(Derivative::Zoo::Chain).to(
        receive(:from_mime_types_for)
          .with(manifest: environment.manifest, config: kind_of(Derivative::Zoo::Configuration))
          .and_call_original
      )
      subject
    end
  ...
  describe "#process_next_chain_link_after!" do
    ...
    context 'when given a derivative that has a next chain link' do
      ...
      context 'when the given derivative is not part of the chain' do
        ...
        it { within_block_is_expected.to raise_exception(Derivative::Zoo::Exceptions::UnknownDerivativeRequestForChainError) }
      end
    end
  end
