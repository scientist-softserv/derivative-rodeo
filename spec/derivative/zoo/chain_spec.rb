...
RSpec.describe Derivative::Zoo::Chain do
  ...
  describe '#each' do
    it 'will yield instances of Derivative::Zoo::Type::BaseType' do
      # NOTE: image yields first, then monochrome which depends on image, then hocr which depends on
      # image
      expect { |b| subject.each(&b) }.to yield_successive_args(
                                           Derivative::Zoo::Type::OriginalType,
                                           Derivative::Zoo::Type::MonochromeType,
                                           Derivative::Zoo::Type::HocrType
                                         )
    end
  end
  ...
  describe '#find_index' do
    ...
    context 'when the derivative exists' do
      let(:derivative) { Derivative::Zoo::Type::MonochromeType }
      it { is_expected.to be_a(Integer) }
    end

    context 'when the derivative does not exist' do
      let(:derivative) { Derivative::Zoo::Types }
    ...
    describe '.call' do
      ...
      context 'with raise_error: true' do
        it 'raises an Exceptions::TimeToLiveExceededError' do
          ...
          end.to raise_exception(Derivative::Zoo::Exceptions::TimeToLiveExceededError)
        end
      end
    end
  end
