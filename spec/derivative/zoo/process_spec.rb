...
RSpec.describe Derivative::Zoo::Process do
  let(:derivative) { double(Derivative::Zoo::Type::BaseType, generate_for: false) }
  let(:environment) { double(Derivative::Zoo::Environment, local_demand!: true, remote_pull: false, process_next_chain_link_after!: false, local_exists?: false) }
  ...
  describe ".call" do
    ...
    context 'when local does not exist in environment' do
      ...
      context 'when nothing returns a handle' do
        let(:exception) { Derivative::Zoo::Exceptions::FailureToLocateDerivativeError.new(derivative: derivative, environment: environment) }
