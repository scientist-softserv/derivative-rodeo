...
describe Derivative::Zoo::Utilities::Image do
  let(:fixtures) { File.join(Derivative::Zoo::GEM_PATH, 'spec/fixtures/files') }
  ...
  describe '#technical_metadata' do
    ...
    context 'with a JP2' do
      ...
      it 'delegates technical metadata to the ImageJp2' do
        expect(Derivative::Zoo::Utilities::ImageJp2).to receive(:technical_metadata_for).with(path: gray_jp2)
        subject
      end
    end
    ...
    context 'with a non-JP2' do
      ...
      it 'delegates technical metadata to the ImageIdentify' do
        expect(Derivative::Zoo::Utilities::ImageIdentify).to receive(:technical_metadata_for).with(path: gray_tiff)
        subject
      end
    end
  end
