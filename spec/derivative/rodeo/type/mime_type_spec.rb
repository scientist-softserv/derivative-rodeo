...
RSpec.describe Derivative::Rodeo::Type::MimeType do
  ...
  describe "#generate" do
    before do
      allow(Derivative::Rodeo::Environment).to receive(:for_mime_type_processing).and_call_original
      allow_any_instance_of(Derivative::Rodeo::Environment).to receive(:start_processing!)
    ...
    it "starts processing for the given mime type" do
      expect_any_instance_of(Derivative::Rodeo::Environment).to receive(:start_processing!)
      instance.generate
      expect(Derivative::Rodeo::Environment).to have_received(:for_mime_type_processing).with(environment: environment)
    end
  end
end
