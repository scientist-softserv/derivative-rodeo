...
RSpec.describe Derivative::Rodeo::Manifest::Original do
  ...
  subject(:manifest) { described_class.new(parent_identifier: 'abc', file_set_filename: 'def.jpg', derivatives: derivatives) }

  it { is_expected.to respond_to :parent_identifier }
  it { is_expected.to respond_to :to_hash }
  it { is_expected.to respond_to :identifier }
  it { is_expected.to respond_to :file_set_filename }
  ...
  describe '#to_hash keys' do
    ...
    it { is_expected.to eq([:name, :parent_identifier, :file_set_filename, :derivatives].sort) }
  end

  describe '#<=>' do
    let(:same_manifest) { described_class.new(parent_identifier: 'abc', file_set_filename: 'def.jpg', derivatives: derivatives) }
    let(:other_manifest) { described_class.new(parent_identifier: 'ghi', file_set_filename: 'def.jpg', derivatives: derivatives) }
