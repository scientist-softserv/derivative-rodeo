...
RSpec.describe Derivative::Rodeo::Process do
  ...
  let(:environment) { double(Derivative::Rodeo::Environment, local_demand!: true, remote_pull: false, process_next_chain_link_after!: false, local_exists?: false) }
  ...
  describe ".call" do
    ...
    context 'when local does not exist in environment' do
      ...
      context 'when nothing returns a handle' do
        let(:exception) { Derivative::Rodeo::Exceptions::FailureToLocateDerivativeError.new(derivative: derivative, environment: environment) }
