...
module SpaceStone
  module Derivatives
    ...
    module DryRun
      ...
      # @param config [SpaceStone::Derivatives::Config]
      #
      # @return [Module]
      #
      # @example
      ...
      #   person.extend(SpaceStone::Derivatives::DryRun.for(method_names: :first_name))
      #
      # @note Given that we're returning a Module the receiver must either include or extend the
      #       caller with the returned module.
      def self.for(method_names:, contexts: [], config: Derivatives.config)
        context_message = ""
