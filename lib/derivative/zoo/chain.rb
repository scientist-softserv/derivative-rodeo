...
module Derivative
  module Zoo
    ...
    class Chain
      ...
      #
      # @return [Chain]
      def self.from_mime_types_for(manifest:, config: Zoo.config)
      ...
      #
      # @return [Chain]
      def self.for_pre_processing(config: Zoo.config)
      ...
      def initialize(derivatives:)
        ...
        @chain = Array(derivatives).each_with_object({}) do |(key, _), hash|
          hash[key.to_sym] = Derivative::Zoo::Type(key.to_sym)
        end
      ...
      #
      # @yieldparam [Derivative::Zoo::Type::BaseType]
      # @see Sequencer
      def each
      ...
      def add_dependencies_to_chain!
        ...
        @chain.values.each_with_object(additional_values) do |type, hash|
          Array(type.prerequisites).each do |prereq|
            ...
            hash[prereq] = Derivative::Zoo::Type(prereq)
          end
        end
